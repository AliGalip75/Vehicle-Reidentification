...
from tool.extract import extract_feature  # tool/extract.py içindeki fonksiyon
...
parser.add_argument("--model_opts", required=True,
                    type=str, help="model saved options (opts.yaml)")
parser.add_argument("--checkpoint", required=True,
                    type=str, help="model checkpoint to load (net_X.pth)")
...
# Query + gallery tüm id'leri aynı class indeksine map’le
...
query_cam = query_df["cam"].to_numpy() if "cam" in query_df.columns else np.array([])
gallery_cam = gallery_df["cam"].to_numpy() if "cam" in gallery_df.columns else np.array([])
...
# remove_classifier=True -> ft_net / ft_net_swin embedding döndürüyor
...
print('Feature extraction complete in {:.0f}m {:.2f}s'.format(
    time_elapsed // 60, time_elapsed % 60))

# label listlerini olduğu gibi bırakıyoruz;
# evaluate.py bunları zaten .mat içinden okuyup [0] ile alıyor.
result = {
...
print("Evaluation unsuccessful, run evaluate.py manually.")
