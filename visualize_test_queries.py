...
parser.add_argument("--no_cams", action="store_true", help="Disable camera filtering")
...
use_cam = ("cam" in query_df and "cam" in gallery_df) and not args.no_cams
...
def show_query_result(axes, query_img, gallery_imgs, query_label, gallery_labels, query_id, gallery_ids):
    ...
    for idx, img in enumerate([query_img] + gallery_imgs):
        ...
        if idx == 0:
            img = query_trans(img)
            label = query_id  # Sorgu için CSV'deki ID
        else:
            # Yeşil çerçeve sadece ID'ler tam eşleşirse
            img = good_trans(img) if query_id == gallery_ids[idx - 1] else bad_trans(img)
            label = gallery_ids[idx - 1]  # Galeri için CSV'deki ID

        ax = axes.flat[idx]
        ax.imshow(img)
        # ID'yi görüntünün üstüne ekle
        ax.text(0.5, 0.95, f"ID: {label}", transform=ax.transAxes, 
