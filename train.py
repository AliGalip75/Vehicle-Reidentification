...
def compute_reid_metrics(features, labels, topk=(1, 5, 10)):
    """
    Basit Re-ID değerlendirme fonksiyonu.
    features: NxD tensor
    labels: N tensor
    """
    ...
    for i in range(num):
        distances = dist[i]
        targets = labels == labels[i]
        sorted_idx = torch.argsort(distances)
        sorted_targets = targets[sorted_idx].cpu().numpy()

        for k in topk:
            if np.any(sorted_targets[:k]):
                correct[k] += 1

        # mAP
        if np.any(sorted_targets):
            relevant = np.where(sorted_targets == 1)[0]
            precision = np.arange(1, len(relevant)+1) / (relevant + 1)
            APs.append(np.mean(precision))

    rank1 = correct[1] / num
    rank5 = correct[5] / num
    rank10 = correct[10] / num
...
def train_model(model, criterion, start_epoch=0, num_epochs=25, num_workers=2):
    ...
    dataloaders = {
        "val": torch.utils.data.DataLoader(image_datasets["val"], batch_size=opt.batchsize, num_workers=num_workers, pin_memory=use_gpu),
        "train": torch.utils.data.DataLoader(image_datasets["train"], batch_sampler=train_sampler, num_workers=num_workers, pin_memory=use_gpu)
    }
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in loader:
                ...
                # store embeddings for val phase
                if phase == 'val':
            ...
            # 🔹 validation: compute Re-ID metrics
            if phase == 'val':
