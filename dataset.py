...
class ImageDataset(Dataset):
    ...
    def get_image(self, idx):
        """
        Belirli bir indeksteki görüntüyü yükler ve RGB formatına dönüştürür. Bu, ham görüntüyü almak için kullanılı
        """
        row = self.df.loc[idx]
    ...
    def __getitem__(self, idx):
        """
        Belirli bir indeksteki görüntüyü ve etiketini döndürür.
        Eğer transform varsa, görüntüye uygulanır.
        Eğer target_transform varsa, etiket üzerinde uygulanır.
        """
        row = self.df.loc[idx]
        pth = os.path.join(self.img_root, row["path"])
        image = Image.open(pth).convert("RGB")
